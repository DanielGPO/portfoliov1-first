...
function Projetos() {
  ...
  return (
    <div
    className='lg:h-[1200px] xl:h-[1000px] md:h-[1200px] h-[1700px] md:pt-2  pt-[400px] '>
      
         <div className='h-[1000px]  relative flex flex-col text-left md:flex-row
     max-w-full justify-evenly mx-auto items-center z-0'>
        <h3 className='  absolute md:top-24 -top-[380px] uppercase tracking-[20px] text-gray-500 text-2xl'
        >Projetos</h3>
        {
        tip 
        ? 
        ...
        viewport={{once: true}}     
        className=' w-full grid md:grid-cols-2 grid-cols-1 lg:mt-[130px] gap-6 z-10 ml-10 md:ml-0'>
          {projetos.map((projeto, i) => (
            <Link to={`${projeto.path}`}>
          <div key={i} className='bg-transparent relative lg:w-[600px] w-[400px] mx-4 group  h-[340px] rounded-lg p-2 hover:cursor-pointer hover:border border-white'>
            <img
             className='w-full  object-cover h-full rounded-lg  hover:cursor-pointer ' src={projeto.image} alt={projeto.name} />

             <div 
              className='absolute hover:cursor-pointer -bottom-3 bg-yellow-600 group-hover:bg-slate-800 w-[300px] ml-12'>
